...
export default async function ResultsPage({
  ...
  const budgetTable = [
  {
    ...
    mech: "자가 대사 활성화",
    action:
      "이번 주 미션: G(주사 요일·식사 시간 고정) / P(저비용 단백질 2회) / S(하루 걸음 수 +20%)",
    value:
      "추가 지출 없이 유지력을 먼저 만든다. 체중이 흔들려도 다시 돌아올 기준이 생긴다.",
  },
  {
    tier: "표준형",
    mech: "근손실 방어",
    action:
      "이번 주 미션: G(결식 금지) / P(HMB 3g + 단백질 보충 1회) / S(하체 근력 15분 × 주 3회)",
    value:
      "감량 중 빠지기 쉬운 근육을 지킨다. 이후 정체기에도 버틸 힘이 생긴다.",
  },
  {
    tier: "집중형",
    mech: "대사 재구성",
    action:
      "이번 주 미션: G(중단 대비 체크리스트 시작) / P(HMB + 단백질 루틴화) / S(근력 주 3–4회)",
    value:
      "단기간 성과보다 약 없이 유지 가능한 몸으로 전환하는 데 집중한다.",
  },
];
  const stageTable = [
  {
    status: "ON",
    def: "지금은 체중을 빼는 시기가 아니라 습관을 만드는 시기다.",
    rec: "체중이 잘 빠질수록 근손실 위험도 함께 커진다.",
    note:
      "이번 주 미션: G(주사 일정 고정) / P(아침 단백질) / S(하체 근력 10분)",
  },
  {
    status: "OFF",
    def: "약물 효과는 끝났지만 체중 관리는 이제부터 시작이다.",
    rec: "중단 직후 4주가 요요를 결정한다.",
    note:
      "이번 주 미션: G(체중 주 1회 기록) / P(간식 단백질화) / S(유산소+근력 병행)",
  },
];
  ...
  const doseTable = [
  {
    ...
    dur: "최소 4주 유지 권장",
    action:
      "이 용량의 핵심은 식사 리듬 유지다. 이번 주 미션: G(주사 요일 고정) / P(아침 단백질 확보) / S(하체 근력 10분)",
  },
  {
    drug: "WEGOVY",
    dose: "2.4mg",
    dur: "최대 유지 용량",
    action:
      "이 용량의 핵심은 근손실 방지다. 이번 주 미션: G(결식 금지) / P(체중×1.2g) / S(근력운동 주 3회)",
  },
  {
    drug: "MOUNJARO",
    dose: "10mg",
    dur: "최소 4주 유지 권장",
    action:
      "이 용량의 핵심은 과속 방지다. 이번 주 미션: G(증량 욕심 금지) / P(매 끼니 단백질) / S(하체 10분)",
  },
  {
    drug: "MOUNJARO",
    dose: "15mg",
    dur: "최대 유지 용량",
    action:
      "이 용량의 핵심은 유지 전환 준비다. 이번 주 미션: G(중단 대비 기록) / P(간식 단백질화) / S(근력 주 3회 고정)",
  },
];
  ...
  const pitfalls = [
  {
    ...
    name: "적응기 고비",
    desc:
      "컨디션이 흔들리면 실패가 아니라 적응 과정이다. 미션: P(한 번에 많이 말고 나눠 섭취) / S(10분 걷기라도 실행).",
  },
  {
    when: "8–12주",
    name: "첫 번째 정체기",
    desc:
      "체중이 멈춰도 루틴이 흔들리면 진짜 정체기다. 미션: G(체중 주 1회 기록) / S(하체 운동 2회 고정).",
  },
  {
    when: "투약 종료 전후",
    name: "리바운드 주의",
    desc:
      "허기가 올라오는 순간이 승부처다. 미션: G(중단 전 2주부터 식사 패턴 고정) / P(간식 단백질화).",
  },
];
...
const weekSegments = [
  {
    when: "0–4주",
    name: "약물 적응기",
    desc: "식욕은 줄지만 체중 변화는 아직 작을 수 있다. 이 시기는 감량이 아니라 준비 단계다.",
    action:
      "이번 주 미션: G(주사 요일 고정) / P(아침 단백질 25g 이상) / S(하체 운동 10분 × 주 3회)",
  },
  {
    when: "5–목표-8주",
    name: "감량 가속기",
    desc: "체중이 빠르게 줄기 시작한다. 이때 근손실을 막지 않으면 이후 요요 위험이 커진다.",
    action:
      "이번 주 미션: G(결식 금지) / P(체중×1.2g 단백질) / S(스쿼트·스텝업 15회 × 3세트)",
  },
  {
    when: "목표-8주–투약 종료",
    name: "가교기",
    desc: "약물 효과가 약해지는 시기다. 생활 패턴을 고정하지 못하면 감량이 멈춘다.",
    action:
      "이번 주 미션: G(투약 간격 기록) / P(간식도 단백질로 대체) / S(근력운동 주 3회 유지)",
  },
  {
    when: "종료 후 +12주",
    name: "유지기",
    desc: "약물 없이 체중이 유지되면 성공이다. 이제 습관이 체중을 지탱한다.",
    action:
      "이번 주 미션: G(체중 주 1회 기록) / P(단백질 식사 루틴화) / S(유산소+근력 병행)",
  },
];
