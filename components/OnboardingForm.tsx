...
export default function OnboardingForm({ onComplete }: { onComplete: (data: any) => void }) {
  const [formData, setFormData] = useState({
    ...
    currentDose: 2.5, currentWeek: 0, startWeightBeforeDrug: 80, // 8, 9 (상세 주수 입력)
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onComplete(formData); }} className="max-w-2xl mx-auto space-y-8 bg-white p-12 rounded-[50px] shadow-xl border border-slate-100">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div><label className={labelClass}>7. 관심 약물</label>
          <select className={inputClass} value={formData.drugType} onChange={e => setFormData({...formData, drugType: e.target.value as any})}><option value="MOUNJARO">마운자로</option><option value="WEGOVY">위고비</option></select>
        </div>
        ...
        {formData.drugStatus === '사용 중' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div><label className={labelClass}>9. 투약 주차</label><input type="number" placeholder="예: 8" className={inputClass} onChange={e => setFormData({...formData, currentWeek: Number(e.target.value)})} /></div>
            </div>
          </div>
        )}
      </div>
      ...
      <div><label className={labelClass}>14. 다이어트 각오</label><textarea className={inputClass} placeholder="예: 요요 없이 건강한 몸을 만들고 싶습니다." onChange={e => setFormData({...formData, resolution: e.target.value})} /></div>

      <button type="submit" className="w-full py-6 bg-blue-600 text-white font-black text-xl rounded-[25px] shadow-2xl hover:bg-blue-700 transition-all">맞춤 로드맵 확인</button>
    </form>
  );
}
